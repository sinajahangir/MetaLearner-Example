...
The results in this code is associated with: USGS 08353000 RIO PUERCO NEAR BERNARDO, NM
"""
#%%
...
class Meta(nn.Module):
    def __init__(self, input_size, hidden_size, output_size=1, dropout_prob=0.4):
...
lr=1e-3
# Create model
model= Meta(input_size, hidden_size)
...
if best_model_state:
    ...
    torch.save(best_model_list[-1], 'Model_Meta_Final_%1.4f_%d'%(lr,hidden_size))
#%%
save_name=f'Meta_{hidden_size}_%1.4f.csv'%(lr)
n_samples=1
for ii in range(0,1):
  ...
  df_temp = pd.DataFrame({
        'date': df_test_tfm_s.iloc[-len(predictions):]['date'],
        'obs': y_test.ravel(),
        'med': predictions.ravel(),
    })
...
for ii in range(0,1):
    pred=np.asarray(df_all.loc[:,'med']).reshape((-1,1))
...
plt.tight_layout()
plt.savefig(f'Meta_USGS08353000_{lr}_{hidden_size}_v1.png')
